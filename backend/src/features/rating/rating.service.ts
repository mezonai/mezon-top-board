...
export class RatingService {
  ...
  async getAllAppRating(query: GetAllAppRatingRequest) {
    ...
    return new Result({
      data: ratings.map((rating) => Mapper(GetAppRatingResponse, rating)),
    });
  }
  ...
  async createRating(userId: string, body: CreateRatingRequest) {
    ...
    if (!user) throw new BadRequestException(ErrorMessages.INVALID_USER);

    const rating = await this.ratingRepository
      .getRepository()
      .findOne({ where: { appId: body.appId, userId } });
    if (rating)
