...
export class BotGeneratorJob implements OnModuleInit {
  ...
  async processJob(tempFileId: string, payload: BotWizardRequest) {
    ...
    if (Array.isArray(payload.commands) && payload.commands.length > 0) {
      const commandsDir = path.join(tempOutput, 'command');
  ...
  private async renderDirectory(srcDir: string, destDir: string, payload: BotWizardRequest) {
  ...
  private async generateCommandFile(commandsDir: string, command: CommandWizardRequest) {
    ...
    const rendered = compile(command);

    const filePath = path.join(commandsDir, `${command.className}.ts`);
