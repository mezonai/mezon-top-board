  ...
  appTranslations: yup.array().of(
    yup.object().shape({
      ...
      headline: yup
        ...
        .when('language', {
          is: AppLanguage.EN,
          then: (schema) => schema.required('validation.headline_required').min(50).max(510),
      ...
      description: yup
        ...
        .when('language', {
          is: AppLanguage.EN,
          then: (schema) => schema.required('validation.full_desc_required'),
          otherwise: (schema) => schema.optional()
        })
    })
  ).required(),
